    ...
    (let ((minimum-overlap nil)
      ...
      (dolist (axis axis-list)
        (let* ((poly0-project-onto-axis (polygon-project-onto-axis (polygon-points-coord poly0) axis)) 
               ...
               ;;             (> poly1-min poly0-max)))
               (result (- (min poly0-max poly1-max)
                          (max poly1-min poly0-min)))
               )
          ;; (format t "axis: ~A~%" axis)
          ;; (format t "~A ~A ~A ~A~%" poly0-min poly0-max poly1-min poly1-max)
          ;; (format t "~A~%" result)
          (when (or (not minimum-overlap)
                  (< result minimum-overlap)) 
              (setf minimum-overlap result)
              (setf minimum-axis axis))
          (when (< result 0)
            (return-from sat nil))
          )
        )
      (format t "overlap: ~A, axis: ~A~%" minimum-overlap minimum-axis)
      (return-from sat t)
      ;(return-from sat (values minimum-overlap minimum-axis))
